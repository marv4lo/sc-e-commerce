...
class FileCorpus extends Corpus {
  ...
  constructor(path, separator = ',') {
    ...
    super(fs.readFileSync(path, 'utf8') // TODO: async?
    .toString().replace(/\r/g, '') // windows introduces \r
    .split('\n').filter(row => row.length > 0).map(row => row.split(separator).map(word => word.trim())), path);
  }
