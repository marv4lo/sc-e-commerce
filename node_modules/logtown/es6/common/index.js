...
function buildMessageOptions(id) {
  const _configRegistry = configRegistry,
  ...
  let options = deepmerge.all([{}, get(configRegistry, `namespaces.${id}`, {}), {
...
function sendMessage(id, level, fallbackToLog, ...rest) {
  ...
  const _applyPlugins = applyPlugins(pluginsRegistry, id, level, calcStats(), ...rest),
        _applyPlugins2 = _toArray(_applyPlugins),
        stats = _applyPlugins2[2],
        args = _applyPlugins2.slice(3);

  wrappers.concat(options.wrappers).forEach(wrapper => {
    ...
    const _applyPlugins3 = applyPlugins(plugins, id, level, stats, ...restOrArgs),
...
function applyPlugins(plugins = [], id, level, stats, ...rest) {
  ...
  const args = Object.freeze(rest.map(arg => Object.freeze(arg))); // mutable context object that passes into plugin function and
...
function addWrapper(wrapper, {
  ...
  if (typeof wrapper === 'object' && !Array.isArray(wrapper) && (typeof wrapper.log === 'function' || Object.keys(LEVELS).some(lvl => typeof wrapper[lvl.toLowerCase()] === 'function'))) {
